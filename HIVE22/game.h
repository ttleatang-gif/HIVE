...
public:
    ...
    void dfs(int depth, const std::vector<std::vector<int>>& path, int player, int id, std::vector<std::vector<int>>& next) {
        if ((depth == 3) && (std::find(next.begin(), next.end(), path[3]) == next.end())) {
            ...
            // 获取当前步的合法移动
            // 为了不修改实际的游戏状态，我们需要模拟移动
            // 这里简化为调用 getLegalOneSteps 并传递两个参数
            // 注意，这里无法真正模拟移动，因此可能需要进一步调整逻辑
            vector<vector<int>> oneSteps = getLegalOneSteps(player, id);
            for (const auto& step : oneSteps) {
    ...
    vector<vector<int>> getCricketSuccessor(int player, int id) {
        ...
        // 检查蟋蟀离开是否会导致不连通
        // Temporarily remove the cricket
    ...
    //-----------------------------------------------------------------------------------------------------
    // 简单的随机 AI
    std::pair<int, vector<int>> getAIRandomMove() {
        vector<pair<int, vector<vector<int>>>> allMoves;

        for (int id = 0; id < 11; ++id) {
            auto successors = getSuccessor(id);
            if (!successors.empty()) {
                allMoves.emplace_back(make_pair(id, successors));
            }
        }
        ...
        random_device rd;
        mt19937 gen(rd());
        uniform_int_distribution<> dis(0, static_cast<int>(allMoves.size()) - 1);

        int idx = dis(gen);
        int id = allMoves[idx].first;
        auto& moves = allMoves[idx].second;

        uniform_int_distribution<> moveDis(0, static_cast<int>(moves.size()) - 1);
        int moveIdx = moveDis(gen);

        return { id, moves[moveIdx] };
    }
...
int main() {
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == Event::MouseButtonPressed && event.mouseButton.button == Mouse::Left && !gameEnded) {
                if (!isAI[game.getCurrentPlayer()]) {
                    ...
                    // 选择要移动的棋子
                    // 这里需要实现选择棋子的逻辑，暂时简化为选择第一个可移动的棋子
        ...
        if (aiThinking && aiClock.getElapsedTime().asSeconds() >= AI_DELAY && !gameEnded) {
            auto [id, move] = game.getAIRandomMove();
            if (id != -1 && !move.empty()) {
